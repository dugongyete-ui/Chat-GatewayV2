...
Aliyun's blocklist, while Node.js fetch and system curl are blocked.
...
# Max retries when Qwen risk-control triggers (哎哟喂被挤爆啦)
MAX_RETRIES = 3
RETRY_DELAY = 3.0
...
def cmd_create(token: str, model: str) -> None:
    r = req.post(
        f"{BASE}/chats/new",
        json={
            "title":     "New Chat",
...
def cmd_chat(token: str, chat_id: str, payload_b64: str) -> None:
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if attempt < MAX_RETRIES - 1:
            ...
            # All retries exhausted — emit error SSE so Node.js gets something
            err_msg = json.dumps({
                "error": {
...
def main() -> None:
    ...
    token = sys.argv[2]
    if cmd == "create":
        cmd_create(token, sys.argv[3])
    elif cmd == "chat":
        cmd_chat(token, sys.argv[3], sys.argv[4])
    else:
